...
- **ROM scan is recursive**: All three scan paths (`bulk_scan`, `/library/scan`, `run_scan` in scheduler) use `subdir.rglob('*')` to find ROMs nested inside subdirectories of a system folder (e.g. `NES/No-Intro/game.nes`). The system name is always taken from the top-level folder under `download_dir`.
- **Archive support (.zip/.7z)**: Both extensions are in `ROM_EXTENSIONS` and are scanned like any other ROM. Hashing extracts to a temp dir, hashes the largest ROM-like file inside, and cleans up — the archive stays on disk. RAHasher handles zips natively; Python fallback uses `_hash_from_archive` in `hasher.py`. `_rom_title()` in `library.py` strips the inner extension from names like `game.nes.zip` → title `game`.
- **`API_GetGameInfoByMD5` field name**: The legacy RA endpoint returns `"ID"` for the game ID; newer RA API docs show `"GameID"`. `lookup_hash` normalises both to `"ID"` so callers always use `match.get("ID")`. When no hash is found the API returns JSON `null` (not `{}`), so `lookup_hash` guards with `isinstance(data, dict)` before calling `.get()`. HTTP 404 is also returned for "not found" and is handled by returning `None` without raising. HTTP 429 raises `RuntimeError` with a clear rate-limit message so the caller's warning log is actionable.
- **RA hash not matching**: Downloaded ROM hashes failing RA verification usually means the dump is not in RA's hash list (different No-Intro dump vs. what RA has accepted). Check `/logs` → category "hash" — the exact hash value is logged so it can be manually looked up on retroachievements.org. The Collection page has a "No RA" filter showing all hashed items that didn't match; the count also appears in the header.
- **Collection `no_ra` filter**: `status == "no_ra"` is a special filter condition (not a real status value) handled separately from the four status literals (verified/found/library/wanted). The backend checks `file_hash` is set and `ra_matched` is False.
