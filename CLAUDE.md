...
- **`API_GetGameInfoByMD5` field name**: The legacy RA endpoint returns `"ID"` for the game ID; newer RA API docs show `"GameID"`. `lookup_hash` normalises both to `"ID"` so callers always use `match.get("ID")`. When no hash is found the API returns JSON `null` (not `{}`), so `lookup_hash` guards with `isinstance(data, dict)` before calling `.get()`.
- **RA hash not matching**: Downloaded ROM hashes failing RA verification usually means the dump is not in RA's hash list (different No-Intro dump vs. what RA has accepted). Check `/logs` → category "hash" — the exact hash value is logged so it can be manually looked up on retroachievements.org.
