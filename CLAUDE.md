...
- **Goals & events tracker** ✅ — `/goals` sets per-game objectives (**master / beat / custom**) optionally grouped under a free-text **event** label, each with a **deadline**. master/beat goals **auto-complete** from the local RA mirror via `services/goals.py::evaluate_goals` (LOCAL, **hardcore-only** — softcore awards never count; run after a dashboard refresh + on every page load); custom objectives (e.g. "finish level 5") are marked done by hand. The page groups by event with a per-event "N of M done" tally, joins each goal live to its `RAGameProgress` row for a progress bar/award badge, and shows three-state deadline badges (done / overdue / due-in-Nd). RA games are attached by reusing `/ra/search` (`mode="goal"`); the `Goal` table is created by `create_all` (no migration). Live-scope `goals`; state in `/api/status.goals` + the `goals` `/api/changes` scope. See `app/routers/CLAUDE.md` → *Goals* and `app/services/CLAUDE.md` → *Goal auto-completion*.
