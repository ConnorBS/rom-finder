...
- `romsfun` (`extensions/romsfun.py`) — searches the **per-system catalog** `/roms/{system-slug}/?q={query}` (the global `?s=` search is result-capped and *buries/misses* titles). **`_normalize_query` replaces apostrophes/backslash with a SPACE** — ROMsFun returns ZERO results for `Don't` *and* for `Dont`; only `Don t` matches (the site's search box even injects a `\` before the apostrophe). **A game page can list more than one ROM** (a USA dump *and* a Europe dump at `/download/{slug}-{id}/1`, `/2`) — only one may be RA's accepted hash, so `get_files()` enumerates every per-file path (`_file_paths()`, capped at `_MAX_FILES`) off the landing page; returning only the first file made hunts exhaust after one wrong-region bad-hash. Each per-file **mirror page embeds its own signed CDN URL** (`?token=…`), so `get_files()` reads it from the HTML via `_parse_cdn_url` (dedup by filename). Optional **`flaresolverr_url` setting** (`configure()`): when set, `_fetch_html()` routes every romsfun.com GET through that FlareSolverr instance so the request originates from *its* IP and clears Cloudflare (cf_clearance is IP-bound — can't be copied here); falls back to direct httpx when blank or if the solver errors. The token-signed CDN host (`sto.romsfast.com`) is not Cloudflare-bound, so the byte stream stays on httpx. **`download_file()` (v1.9.0) resolves the token and streams it inside ONE httpx session**: `_resolve_cdn_url` visits the mirror page (setting the session cookie), mints a fresh signed URL via **admin-ajax** (`k_get_download`, *primary* — `Referer`-bound but works fine on direct httpx, which is the path that downloaded reliably pre-1.8.0) and falls back to the page-embedded token (then a FlareSolverr fetch if the direct page was CF-blocked); `_stream_to` then streams **atomically** (`.part`→`os.replace`, short-body guard). The anti-leech CDN 403s/429s **intermittently even on a good file**, so it retries `_DOWNLOAD_ATTEMPTS` (3×) with backoff and raises the typed `SourceForbiddenError`/`SourceRateLimitError` (not a bare `RuntimeError`). **The v1.8.0 regression** that broke apostrophe titles (e.g. *Don't Fear the Dark*, *Louis's Punch-Out*): it streamed from a *fresh, cookie-less* client (and could have minted the token via FlareSolverr's IP) — a different session/IP than the one that resolved it → the CDN 403'd.
