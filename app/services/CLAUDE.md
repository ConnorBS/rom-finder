...
`recompute_duplicates(session)` tags redundant `LibraryEntry` copies via `duplicate_of` (canonical sibling's id; `None` = canonical/unique). It's a **full rebuild** (clear then re-derive) so resolved/removed dups clear — never append-only. Entries are unioned (union-find) when they share **identical hash**, the same **`(ra_game_id, disc_no)`**, or the same **normalized title+system**; each group's canonical = RA-matched, then non-archive, then lowest id. **Subset rules (user requirement):** different discs of a multi-disc game never merge (disc/side marker parsed from the name; the title key keeps the marker), and `.bin`/`.img` tracks are excluded entirely (a track is a component of its `.cue` — tagging/deleting it would break the disc). LOCAL only (no RA calls); cheap enough to run after every scan/rehash/verify pass.
