...
`refresh()` builds the **local mirror** the dashboard reads from (so browsing makes zero RA calls). It fetches profile + every unlock (`get_achievements_earned_between`, member-since→now in `ra_dashboard_window_days` windows, de-duped on `(achievement_id, hardcore)`) + paginated completion (`get_user_completion_progress`) + awards, then **replaces `ra_achievement`/`ra_game_progress` in one transaction** — wholesale replace is how retroactive RA changes reconcile. **Event-hub backfill (step 2b):** `get_achievements_earned_between` **under-reports event "games"** (AotW/Roulette) — it returned only 14 of the user's 28 earned AotW unlocks, so those goals never auto-completed. So after the windowed pass, refresh pulls **each tracked event hub** (distinct `GoalEvent.ra_game_id`) via `get_game_user_progress` (`API_GetGameInfoAndUserProgress`) and `setdefault`-merges any earned unlock the windows missed (the windowed entry wins where present). Diagnose the gap with `GET /api/diag/user-game-progress?game_id=<hub>` (`earned_but_missing_from_mirror`). Fresh `Session` per await-gap; all RA calls use the shared 2 req/s `_limiter`; progress reported via `activity_store` (`ra-sync`); stamps `ra_dashboard_last_sync`. **Manual only** (no scheduler task). New `RAClient` user methods it relies on: `get_user_profile`, `get_achievements_earned_between` (Unix `f`/`t`), `get_user_completion_progress` (paginated), `get_user_awards`. At the end it also runs `mastery.sync_library_awards` + `subsets.recompute_subset_flags` + `goals.evaluate_goals` (all LOCAL) so the collection's badges + goal completions reflect the fresh mirror immediately.
