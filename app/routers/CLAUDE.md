...
- **Sub-categories** (`GoalCategory`): `POST /goals/category` (create/upsert: `event_name`,`name`,`deadline`,`notes`), `POST /goals/category/edit` (`event_name`,`old_name`,`name`,`deadline`,`notes` — rename re-points its goals), `POST /goals/category/delete` (goals revert to uncategorized). All return `HX-Refresh`. `_build_group` splits each event into **sub-sections** (one per category + an uncategorized section) ordered by **closest due date** (a category by its date, the uncategorized section by its earliest game). Each category is collapsible (click the name or chevron; state in `localStorage`). `notes` renders below the header via **`_render_notes`** — a tiny SAFE markdown (escape → `**bold**`/`*italic*`/`` `code` ``/`[label](http…)`/newlines), empty = no element.
