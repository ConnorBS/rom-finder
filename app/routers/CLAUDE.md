...
`""` (delete-swap). **`master`/`beaten` are hardcore-only** (see `services/CLAUDE.md` → goals.py).
...
- **`GET /api/status`** → `{version, rahasher:{available,path,bin}, db:{library_total, library_ra_matched, no_ra, library_unhashed, wanted_*, downloads_*}, scheduler:{scan,hash,autodiscover → {enabled,time,last_run}}, verify:{in_progress,paused_until,last_run}, dashboard:{achievements,games,last_sync}, goals:{total,active,completed,overdue,custom}, sources[], extensions[], recent_errors:{window_hours,count,latest[]}}`. `version` is `APP_VERSION` (the deployed git SHA). `dashboard` reports the RA mirror's local counts + last manual sync. Each section is independently `try`-guarded — a failing section returns `{"error": "..."}` instead of blanking the whole report.
