...
**Auto-hunt downloads now appear as cards too.** `hunter.auto_hunt` creates a transient `Download` (`status=downloading`, `hunt_task_id="hunt-{wanted_id}"`, `file_path=None`) per attempt with a real `progress_callback`, so the Downloads page + tray show live hunt-download progress and a **Cancel** button (`hx-post /activity/cancel/{hunt_task_id}` → the hunt stops at the next candidate and removes the row). On verify the row is reused (→ `pending_approval`, `hunt_task_id` cleared); on bad_hash/fail it's deleted (the `HuntAttempt` is the record). See `app/services/CLAUDE.md` → hunter.
