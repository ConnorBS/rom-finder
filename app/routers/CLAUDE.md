...
**Auto-hunt downloads now appear as cards too.** `hunter.auto_hunt` creates a transient `Download` (`status=downloading`, `hunt_task_id="hunt-{wanted_id}"`, `file_path=None`) per attempt with a real `progress_callback`, so the Downloads page + tray show live hunt-download progress and a **Cancel** button (`hx-post /activity/cancel/{hunt_task_id}`). Cancel **aborts the in-flight download within ~2s** (the hunt polls the flag during the download, not only between candidates) and **stops the hunt without the torrent/usenet fallback** (it no longer parks the game in `awaiting_external` — the reported "can't cancel from the queue" bug). On verify the row is reused (→ `pending_approval`, `hunt_task_id` cleared); on bad_hash/fail it's deleted (the `HuntAttempt` is the record); on cancel it's deleted with no `HuntAttempt`. See `app/services/CLAUDE.md` → hunter.

**Orphaned-card reaper (un-cancellable "Downloading 0%" after a redeploy).** The above only cleans up if the hunt coroutine *runs to its `finally`* — but a **hard restart** (the SIGKILL Watchtower issues on a redeploy) kills the coroutine without it, stranding the transient card; and the in-memory hunt task is gone too, so its Cancel button (which only signals that task — `POST /activity/cancel/{task_id}` → `activity_store.cancel`) becomes a permanent no-op (the user's "Learning with the Pooyoos keeps downloading and I can't cancel it"). Fixed three ways, all via `hunter.reap_orphaned_hunt_downloads(task_id=None)` (deletes transient `downloading`/`hashing`/`verifying` rows with a `hunt_task_id` and **no live `ExternalDownload` backing them**): (1) **startup** — `main.py` lifespan reaps every orphan (a restart kills *all* hunt coroutines); (2) **Cancel fallback** — the cancel endpoint checks `activity_store.is_active(task_id)`; if the hunt task is already gone it reaps that `hunt-{id}`'s orphaned card directly instead of no-opping; (3) the hunt's own `finally` calls the same helper. External torrent/usenet cards are preserved (the scheduler poll resumes them across restarts), and the poller no longer **resurrects** a card the user deleted (`external_hunt._ingest` guard).
...
### Multiple directories — directory filter, cross-dir-dup filter, move endpoints
The collection combines every `LibraryRoot` (see `services/CLAUDE.md` → *Library roots*).
`_build_collection` joins each item to its root → `root_label` (a `⌂` badge on cards/rows, shown only
when >1 root) and computes `cross_dir_dup` (a duplicate group spanning >1 directory → a fuchsia ⧉ badge).
Filters: **`?root=<id>`** (a `#col-root` `<select>`, threaded through every nav link + `hx-include` like
`#col-system`; a hidden `#col-root` input stands in when there's a single root) and **`status=cross_dir_dup`**
(a computed filter, like `duplicate`). Counts add `cross_dir_dup`. **Move endpoints** (`collection.py`):
- **`POST /collection/library/{id}/move`** (Form `dest_root_id`) — single move via `_relocate`. **No
  `Depends(get_session)`** (opens its own sessions, so the pool isn't held across the file copy);
  returns `HX-Refresh: true` on success or an inline error (read-only source/dest, name conflict, same-dir).
- **`POST /collection/bulk/move`** (Form `library_ids`, `dest_root_id`) — background `_bulk_move`
  (activity batch `move-batch`, per-file fresh session); selection-bar **Move to →** dropdown + the
  detail panel's per-entry **Move to** control post here. `_relocate` reuses the `.cue`/`.gdi` sibling
  logic and re-runs `recompute_duplicates`. **Settings** manages directories:
  `POST /settings/roots/{add,/{id}/remove,/{id}/primary,/{id}/readonly,/{id}/mapping}` (the
  *Library Directories* section; per-directory folder→console mapping saves to `LibraryRoot.folder_map`).

### Collection `bad_chd` filter (CHD codec)
`status == "bad_chd"` filters to entries whose `LibraryEntry.chd_codec in ("cdzs","zstd")` — CHDs on
the Zstandard codec that RetroArch can't hash for achievements. Computed from the column stamped by
`services/chd_format.run_chd_check` (the opt-in **CHD format check** scheduler `chdcheck` task / its
Run-now button; convert when `chdman` is present, else flag-only). Surfaces as a `⚠ CHD codec` rose
badge (card + list), a `CHD codec` filter chip, a header count, and `/api/status.chd`
(`{enabled, chdman_available, checked_chd, bad, last_run}`). See `app/services/CLAUDE.md` → *CHD format check*.
...
- **`POST /api/wanted`** (`WantedAddRequest{ra_game_id, game_title, system, system_id, hunt}`) — adds to Wanted; returns `{status: "added"|"exists", id, game_title, hunting}`. System is resolved server-side via `canonical_system(system, system_id)` (the RA console id wins), so a wrong/empty scraped name is fixed at the source. **`hunt: true`** (the extension panel's "Hunt immediately after adding" checkbox) backgrounds `hunter.auto_hunt(id)` right after adding — also honoured on the `exists` path (unless already verified or a hunt is already running). **Cover gotcha:** the cover BackgroundTask is `_fetch_cover(game.id, ra_game_id, game.game_title, system)` — pass the **title + canonical system**, never the RA creds. Passing `username` as `game_title` made title-based cover sources (SteamGridDB) fetch one identical image for *every* extension-added game (regression fixed; `tests/test_api_extension.py` guards it).
...
- **`POST /api/goal`** (`GoalAddRequest{ra_game_id, game_title, system, system_id, objective, achievement_id, achievement_title, event_name, category, deadline}`) — creates a `Goal` from the extension. `category` (optional) assigns the goal to a sub-category within the event (both content.js's game-goal widget and achievement.js post it). `objective` ∈ `master|beaten|achievement`; **`achievement` requires `achievement_id`** and auto-completes on a HARDCORE unlock. System resolved via `canonical_system`, title via `clean_title`, `deadline` parsed `YYYY-MM-DD` (""→None). De-dups on (`ra_game_id`,`objective`,`achievement_id`) → `{status:"exists"}`; else `{status:"added", id, objective}`. For an **achievement** goal it backgrounds `goals._enrich_achievement_goal` (RA `API_GetGameExtended` → canonical title + description + **badge image** stored as an absolute `cover_path`); for master/beat it reuses `covers/{ra_game_id}.png` or backgrounds `goals._fetch_cover_goal`. The achievement-page content script (`rom-finder-extension/achievement.js`, match `/achievement/*`) posts here (objective=achievement). A game page posts a **game-level** goal here too: `content.js`'s own **🎯 Set Goal** floating button (`buildGameGoalWidget`, game pages only — gated `!onAchPage` so it never double-stacks with achievement.js) opens a mini panel (Beat/Master + event datalist + deadline) → `objective=beaten|master`. It is **NOT** inside the ROM Finder panel — the in-panel goal row was removed once goal-setting became its own stacked button (manifest 1.8.4). So both page types show **🎯 Set Goal** (bottom:68px) stacked above **🎮 ROM Finder** (bottom:20px).
...
- **`POST /api/import-event`** (`EventImportRequest{ra_game_id|event_ref, event_name, deadline, include_completed}`) — JSON twin of `POST /goals/import-event` for the extension's **event-page panel** (`rom-finder-extension/event.js`, match `/event/*`). Reuses `events.sync_event` (one `API_GetGameExtended` call; records an auto-sync `GoalEvent`); returns `{status, created, skipped_*, event}`. `ra_game_id` (on an `/event/` page) or a pasted `event_ref` URL/id.
- **`GET /ra/events/search?q=`** (`games.py`, HTMX) — search RA events by NAME (`RAClient.search_events` → `get_game_list(101)` Events console + title filter) → `partials/ra_event_results.html`; each result's button calls `prepImportEvent(id,title)` to fill the Goals page's Import-event form. Lets you find an event without pasting a URL/id.
...
**Achievement-page stacking (1.8.3):** `content.js` now injects on **`/achievement/*`** too (manifest match + `background.js` re-inject, delayed 500ms so RA's breadcrumb renders first), deriving the **parent game** from the page's `/game/` link (`scrapeGameLink`; `getGameTitle` prefers that link's text since the `<h1>` is the achievement). So an achievement page shows the **🎮 ROM Finder** game panel (wanted/search) at `bottom:20px` with `achievement.js`'s **🎯 Set Goal** button stacked above it at `bottom:68px` (panel id `rf-ach-panel-root` vs `rf-panel-root` — no collision). `content.js`'s own game-goal Set Goal button is gated `!onAchPage`, so it doesn't double-stack here.
...
### Sub-categories, failed status & custom card art
- **Sub-categories** (`GoalCategory`): `POST /goals/category` (create/upsert: `event_name`,`name`,`deadline`,`notes`,`ra_game_id`,`system`), `POST /goals/category/edit` (`event_name`,`old_name`,`name`,`deadline`,`notes`,`ra_game_id`,`system` — rename re-points its goals; clearing `ra_game_id` detaches the game), `POST /goals/category/delete` (goals revert to uncategorized). All return `HX-Refresh`. `_build_group` splits each event into **sub-sections** (one per category + an uncategorized section) ordered by **closest due date** (a category by its date, the uncategorized section by its earliest game). Each category is collapsible (click the name or chevron; state in `localStorage`). `notes` renders below the header via **`_render_notes`** — a tiny SAFE markdown (escape → `**bold**`/`*italic*`/`` `code` ``/`[label](http…)`/newlines), empty = no element.
- **Game-backed sub-category** (`ra_game_id`): a category can be attached to an RA game **by id or manual search**. The add/edit form embeds a compact RA game picker (`cat_game_search` macro → `GET /ra/search?mode=category` → `partials/ra_game_results.html` "Use →" button → `selectCategoryGame(btn,…)` fills the *closest form*'s `name`/`ra_game_id`/`system`). On submit, `create_category`/`edit_category` store the id+console and background **`_enrich_category`** (NOT in the request — sync route): it resolves the title (only when none was typed, via `RAClient.get_game_info` → re-points goals from the `Game #id` placeholder), console, and box art (reuse `covers/{ra_game_id}.png` on disk, else fetch via the shared `_fetch_game_cover_file`). The header then renders the box-art thumbnail + console·↗ link to RA. Detaching (empty id) clears `cover_path`/`system`.
- **Per-event category datalist** (`_category_names_for_event`): a goal card's `category` field (edit + copy forms) suggests **only its own event's** sub-categories via a per-card `<datalist id="category-names-{goal.id}">` (built from `event_categories`, threaded into the card both on the page — via the `{% with %}` include — and on single re-render in `_card_ctx`). Replaces the old global `#category-names` datalist that mixed every event's categories.
- **Failed**: `POST /goals/{id}/fail` sets `GoalStatus.failed` (manual; `evaluate_goals` only touches `active`, so it never auto-fails or un-fails). Failed goals are **hidden** unless `?show_failed=1` (a toolbar toggle), render a red ✗ overlay, and are EXCLUDED from the event tally (`_build_group` tallies `live` = non-failed). `reopen` un-fails. Default page **`sort=due`** so games + categories order by closest due date.
- **Custom art**: `POST /goals/{id}/edit` also takes `category`/`display_text`/`icon`/`icon_color` (icon validated against `goals.GOAL_ICONS` — a curated tintable glyph set). `POST /goals/{id}/image` (multipart `image`) saves `covers/goal_{id}.{ext}` → `Goal.custom_image` (refused if `covers_dir_readonly`); `POST /goals/{id}/image/clear` removes it. Card-art priority: `custom_image` > `display_text`+`icon` > cover/badge > letter.
- **Copy** (clone, not move): `POST /goals/{id}/copy` (Form `event_name`,`category`) inserts a NEW `Goal` cloning the source's tracking + art fields (`_COPY_FIELDS`) into the target event/sub-category, **leaving the original in place** — so the same objective can live in several events/categories. The copy starts `status=active`/`auto=False`/`completed_at=None` (auto-tracked master/beat/achievement copies self-heal to completed on the next `evaluate_goals` pass; a copied *custom* goal is a fresh to-do). De-dups against an identical goal already in the **target** group (same objective + ra_game_id + achievement_id + custom_text + display_text — NULL id/ach render as IS NULL so custom goals match on text), so copying onto its own group no-ops. Any uploaded `custom_image` is copied to the clone's own `goal_{newid}.{ext}` (so editing/clearing the source's image can't break the copy). Returns `HX-Refresh` so the new card lands in its group. Card button: **⧉ Copy** (toggles an inline event+category form), beside **✎ Edit**.

`/goals` tracks per-game event objectives (master/beat/custom) with deadlines. Routes:
`GET ""` (list), `POST /add` (RA-linked), `POST /add-custom`, `POST /{id}/complete`,
`POST /{id}/reopen`, `POST /{id}/fail`, `POST /{id}/edit`, `POST /{id}/copy`, `POST /{id}/image[/clear]`,
`POST /category[/edit,/delete]`, `DELETE /{id}`. All sync (`Depends(get_session)`) —
**no async-network route here**: attaching an RA game reuses the existing async `GET /ra/search`
(`games.py`) with a new **`mode="goal"`** branch in `partials/ra_game_results.html` (a "Select →"
button that JS-fills the goals add-form's hidden `ra_game_id`/`game_title`/`system`), so the
session-across-await pitfall is sidestepped. The list page calls `goals.evaluate_goals(session)`
before rendering (LOCAL), groups goals by `event_name` ("" = "No event", rendered last) with a
"N of M done" tally, and joins each goal live to its `RAGameProgress` row for the progress bar/
award badge. `_fetch_cover_goal` (mirrors `wanted._fetch_cover`) fetches a cover into the shared
...
- **`GET /api/status`** → `{version, rahasher:{available,path,bin}, db:{library_total, library_ra_matched, no_ra, library_unhashed, wanted_*, downloads_*}, scheduler:{scan,hash,autodiscover → {enabled,time,last_run}}, verify:{in_progress,paused_until,last_run}, dashboard:{achievements,games,last_sync}, goals:{total,active,completed,overdue,custom,achievement}, sources[], extensions[], recent_errors:{window_hours,count,latest[]}}`. `version` is `APP_VERSION` (the deployed git SHA). `dashboard` reports the RA mirror's local counts + last manual sync. `chd` (`{enabled, chdman_available, checked_chd, bad, last_run}`) reports the CHD format check's opt-in state + how many owned CHDs are on the RA-incompatible Zstandard codec. `directories` is a list of the registered `LibraryRoot`s (`{id, label, path, primary, readonly, rom_count}`). Each section is independently `try`-guarded — a failing section returns `{"error": "..."}` instead of blanking the whole report.
- **`GET /api/logs?level=&category=&since=&limit=`** → JSON array of `AppLog` rows (newest first; `limit` 1–1000, `since` is ISO-8601 UTC).
- **`GET /api/changes`** → `{scope: "<fingerprint>"}` for `library, wanted, goals, downloads, hunts, logs, scheduler, dashboard`. The **`goals`** scope is `count:completed:cover_len_sum:max(updated_at):event_total:event_touch:cat_total:cat_cover_len:cat_touch` (add/complete/reopen/edit/auto-complete bump `updated_at`; a background cover write only sets `cover_path`, hence the length sum — same reasoning as `wanted`; the `GoalCategory` tail makes a background game-enrichment landing on a sub-category morph the page). The cheap change-signal the `base.html` live-update poller diffs to decide when to morph a page in place (see `templates/CLAUDE.md` → *Live in-place updates*). Each scope is one tiny `COUNT`/`MAX`/`SUM` fingerprint, independently `try`-guarded (`err:<msg>` on failure, never a 500). **`library` is an aggregate** (`count`, hashes, `SUM(cast(ra_matched,int))`, cover-length sum, dups, missing, saves, award/subset length sums, **`SUM(root_id)`** so a move between directories morphs the page, plus `MAX(added_at/hashed_at/ra_checked_at/missing_at)`) because `LibraryEntry` has **no single `updated_at`** — cover/award/subset/dup/move writes bump no timestamp. **`downloads` deliberately EXCLUDES progress** (count + status counts + `MAX(created_at)`, not `updated_at`) so it only moves on structural change; live progress bars stay the job of the per-item `/downloads/{id}/status` poll. **Gotcha:** `SUM` over a Boolean column gets Boolean result-processing (collapses to True/False) — cast bool→`Integer` so the count stays granular (`tests/test_changes_signal.py` guards this).
